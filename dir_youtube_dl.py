...
'''
download youtube play list
by write the URL of the playlist
or pyperclip source that copied URL
'''
...
def youtube_instruction():
    ...
    if len(command)==1:
        g.content="Command had lack of argument, complete the Sentence"
    elif command[1]=='-u':
        dl_u()
    else:
    ...
    if response:
        ...
        for i, url in enumerate(video_urls):
            try:
                ...
                ###print *3
                print(fmt_data["extension"])
                print(fmt_data["profile"])
...
def download(url, filename):
    ...
    with open(filename, 'wb') as dst_file:
        while True:
            ...
            print(str(_buffer)+' '+str(bytes_received)+' '+str(download_size))
